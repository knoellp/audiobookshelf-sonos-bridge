...
func setRequiredEnvWithTempConfig(t *testing.T) string {
	t.Helper()
	tmpDir := t.TempDir()
	os.Setenv("BRIDGE_ABS_URL", "http://audiobookshelf:13378")
	os.Setenv("BRIDGE_PUBLIC_URL", "http://192.168.1.100:8080")
	os.Setenv("BRIDGE_CONFIG_DIR", tmpDir)
	return tmpDir
}

func TestLoad_RequiredFieldsMissing(t *testing.T) {
	clearEnv()
	// Set a temp config dir so session secret auto-generation can work
	os.Setenv("BRIDGE_CONFIG_DIR", t.TempDir())
	...
	// Note: SESSION_SECRET is auto-generated, so no error expected for it
}
...
func TestLoad_SessionSecretAutoGenerated(t *testing.T) {
	clearEnv()
	tmpDir := setRequiredEnvWithTempConfig(t)
	...
	// Session secret should be auto-generated (64 hex chars = 32 bytes)
	if len(cfg.SessionSecret) != 64 {
		t.Errorf("expected 64-char auto-generated secret, got %d chars", len(cfg.SessionSecret))
	}

	// File should exist
	keyPath := filepath.Join(tmpDir, "session.key")
	if _, err := os.Stat(keyPath); os.IsNotExist(err) {
		t.Error("expected session.key file to be created")
	}
	...
	if cfg2.SessionSecret != cfg.SessionSecret {
		t.Error("expected same session secret on second load")
	}
}

func TestLoad_SessionSecretFromEnvOverridesFile(t *testing.T) {
	clearEnv()
	tmpDir := setRequiredEnvWithTempConfig(t)

	// First, auto-generate a secret
	cfg1, err := Load()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	autoGenerated := cfg1.SessionSecret

	// Now set an explicit secret via env
	clearEnv()
	os.Setenv("BRIDGE_ABS_URL", "http://audiobookshelf:13378")
	os.Setenv("BRIDGE_PUBLIC_URL", "http://192.168.1.100:8080")
	os.Setenv("BRIDGE_CONFIG_DIR", tmpDir)
	os.Setenv("BRIDGE_SESSION_SECRET", "explicit-secret-that-is-at-least-32-chars")
	...
	// Explicit env value should be used, not the auto-generated one
	if cfg2.SessionSecret != "explicit-secret-that-is-at-least-32-chars" {
		t.Errorf("expected explicit secret, got: %s", cfg2.SessionSecret)
	}
	if cfg2.SessionSecret == autoGenerated {
		t.Error("env secret should override auto-generated secret")
	}
}
